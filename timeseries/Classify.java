...
public class Classify {
	 ...
	 * 
	 */
	public void classifyInMemory(String trianPath, String testFileName, int k,
			String writeFileName) throws IOException {
		final int max_m = 301;
		final int max_n = 501;
		Similarity similar = new Similarity(max_m, max_n);
		List<String[]> testStringData = TextFile
				.readAsMatrix(testFileName, ",");
		Map<String, List<double[]>> testData = formatTestData(testStringData);// 将测试数据导入内存
		...
		for (File trianFile : dir.listFiles()) {// 将训练数据导入内存
		...
		System.out.println("文件读完,准备测试...");
		for (Map.Entry<String, List<double[]>> testPerDevice : testData
				.entrySet()) {
			int segmentId = 0;
			for (double[] segmentQ : testPerDevice.getValue()) {
				double[] minDistances = new double[k];// 最短个k个距离
				...
				for (Map.Entry<String, List<double[]>> trainPerDevice : trianData
						.entrySet()) {
					int trianSegId = 0;
					for (double[] segmentC : trainPerDevice.getValue()) {
						...
						double distance = similar.constraintDtw(segmentQ,
								segmentC, 0.1f);
						motifyKnear(minDistances, minLables, trianSegIds,
								distance, trainPerDevice.getKey(), trianSegId++);// 看是否在min
																					// k中
					}
				}
				...
				for (int i = 0; i < k; i++) {
					System.out
							.print(minLables[i] + "$" + trianSegIds[i] + "\t");
					out.print(",");
	...
	/**
	 * 检验distance是否比已经保存的min k小，如果更小，则更小min k
	 * 
	 * @param minDistances
	 * @param minLables
	 * @param trianIds
	 ...
	 * @param trianSegId
	 */
	private void motifyKnear(double[] minDistances, String[] minLables,
			int[] trianIds, double distance, String deviceId, int trianSegId) {
		...
		if (in == false)
			return;
		for (int i = 1; i < len; i++) {// 从第二个元素开始排序，前面的一个元素默认有序了
			if (minDistances[i] > minDistances[i - 1]) {
				double sentry = minDistances[i];// 复制为哨兵
				String id = minLables[i];
				int segId = trianIds[i];
				int j;
				for (j = i - 1; j >= 0 && sentry > minDistances[j]; j--) {
					minDistances[j + 1] = minDistances[j];// 记录后移
					minLables[j + 1] = minLables[j];// 记录后移
					trianIds[j + 1] = trianIds[j];
				}
				minDistances[j + 1] = sentry;// 插入到适当的位置
				minLables[j + 1] = id;
				trianIds[j + 1] = segId;
			}
		}
	...
	public static void main(String[] args) throws IOException {
		...
		classifier.classifyInMemory(".\\data\\train_cos_avg",
				".\\data\\test.csv", 25, "data\\kNear2.csv");
	}
}

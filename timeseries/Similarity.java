...
public class Similarity {
	...
	public double constraintDtw(double[] segmentQ, double[] segmentC,
			float limitQ, float limitC) {
		...
		for (int j = 0; j < n; j++) {
			...
			if (lower > 0) {// 设置限制线
				for (int lowerWall = bSize; lowerWall > 0 && lower > lowerWall
						&& lower - lowerWall < m - 1; lowerWall--)
					disMatrix[lower - lowerWall][j] = Double.MAX_VALUE;
			}
			if (upper < m)
				for (int upperWall = 0; upperWall < bSize
						&& upper + upperWall < m; upperWall++)
			...
			for (int i = lower; i < upper; i++) {
				...
				if (i == 0 && j == 0)
					...
					int minIndex = getMinIndex(disMatrix[i][j - 1],
							disMatrix[i - 1][j - 1], disMatrix[i - 1][j]);
					switch (minIndex) {
					case 0:
						disMatrix[i][j] = disMatrix[i][j - 1] + distance;
						break;
		...
		return trimedDtw(m, n);// 当候选序列较长时修剪
	}
	...
	private double trimedDtw(int lenQ, int lenC) {
		...
		int j = lenC - 1;
		int upI = 0, upJ = 0;
		while ((i > 0 && j == lenC - 1) || (j > 0 && i == lenQ - 1)) {// 右端对齐
			upI = i;
			upJ = j;
			int minIndex = getMinIndex(disMatrix[i][j - 1],
					disMatrix[i - 1][j - 1], disMatrix[i - 1][j]);
	...
	private class Bounder {
		...
		double interceptC;// 纵坐标截距
		double interceptQ;// 横坐标截距
		private int lenQ;
